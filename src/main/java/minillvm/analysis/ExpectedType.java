...
public class ExpectedType {
    ...
    public static Type expectedType(Operand operand) {
        ...
        // implemented the case: ConstList
        else if (parent instanceof ConstList) {
            if (parent.getParent() instanceof ConstStruct) {
                // find the index of operand
                for (int i = 0; i < parent.size(); i++) {
                    if (operand == parent.get(i)) {
                        TypeStruct struct = ((ConstStruct) parent.getParent()).getStructType();
                        return struct.getFields().get(i).getType();
                    }
                }
            }
        }
